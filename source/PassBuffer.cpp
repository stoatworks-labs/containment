                         ...
                         Wrap requestedWrap )
{
	...
	if( fboID != 0 && width == requestedWidth && height == requestedHeight
	    && internalColorFormat == format && sampling == requestedSampling && wrap == requestedWrap )
		return true;
...

	{
		...
		//Clamp to edge unless asked. The default is GL_REPEAT, and a blur tap
		//that wraps puts the right-hand edge of a picture into the left-hand
		//one. The surface is the exception: its domain really is periodic.
		const GLint mode = wrap == Wrap::Repeat ? GL_REPEAT : GL_CLAMP_TO_EDGE;
